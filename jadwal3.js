...
function getkota (search) {
    return fetch('https://api.myquran.com/v1/sholat/kota/cari/'+ search )
        ...
        }).catch(e => hasilSearch.innerHTML = `<p class="list-group-item list-group-item-action">Maaf Kota Tidak Ketemu</p>`)
}
...
//dapatakan jadwal sholat 
function getJadwalSholat () {
    return fetch('https://api.myquran.com/v1/sholat/jadwal/'+ localStorage.getItem("id") + getTanggalBulan())
        .then(Response => Response.json())
        .then(Data => {
...
function tampilKotaDanTanggal () {
    ...
    document.querySelector(".dzuhur").textContent = localStorage.getItem("dzuhur")
    document.querySelector(".ashar").textContent = localStorage.getItem("ashar")
    document.querySelector(".maghrib").textContent = localStorage.getItem("Maghrib")
